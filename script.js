function calculate(){
    ...
    data.forEach((element, index) => {
        BLdata[index] = data[index].map(function(el) {
            return el*0.25;
        })
    });
    // console.log(BLdata)
    ...
    // console.log(BLdata[0].reduce(reducer));
    BLdata.forEach((element, index) => {
    ...
    // console.log(BL)
    var PlaceBL = document.getElementById('BL');
    PlaceBL.innerText = BL


    // Гермейера
    var minimumsG = [Math.min(...BLdata[0]), Math.min(...BLdata[1]), Math.min(...BLdata[2]), Math.min(...BLdata[3])]
    ...
    for (let row = 0; row < 4; row++) {
        SevData[row][0] = [max1col- data[row][0], max2col- data[row][1], max3col-data[row][2], max4col-data[row][3]]
    }
    console.log(SevData)
    var maximumsSev = [Math.max(...SevData[0][0]), Math.max(...SevData[1][0]), Math.max(...SevData[2][0]), Math.max(...SevData[3][0])]
    console.log(SevData[0][0])
    console.log(SevData[1][0])
    console.log(SevData[2][0])
    console.log(SevData[3][0])
    console.log(Math.max(...SevData[0][0]))
